 	  	...
 	  	<view class="flex">
 	  					<view class="flex">
 	  						<view class="logistics-number-scroll-li"  v-for = "(item,index) in title" :key="index"  >
 	  							<view class="logistics-number-top" @click="choo"  :class="[navv==index?'activee':'']"  :data-id="item.id" :data-index="index">礼包{{item.index}}</view>
 	  							<view class="logistics-number-commodity margin-auto">
 									<image class="img" :src="$utils.imageUrl(item.goodsinfo[0].head_img)" v-if="item.goodsinfo[0].head_img" mode=""></image>
 	  ...
 	  <!-- <view  v-if="orderinfo.status==0 || orderinfo.status==1 || orderinfo.status==2 || orderinfo.status==3"> -->
 		  ...
 		  <view style="margin-bottom: 20rpx;">
 		  					 ...
 		  					 <view class="order-add flex-vertically"  @click="add">
 		  						 ...
 		  						 <view class="order-add-information">
 		  							 ...
 		  							 <view class="order-address">{{orderinfo.province}}  {{orderinfo.city}}  {{orderinfo.county}}  {{orderinfo.address}} </view>
 		  						 </view>
 		  						 <!-- 修改时间 -->
 		  ...
 		  <view class="order-rdetails-information  order-rdetails-information-over">
 		  	<view class="order-rdetails-information-content">
 		  		...
 		  		<view class="order-rdetails-information-li flex-vertically" v-if="orderinfo.status==0 || orderinfo.status==1 || orderinfo.status==2 || orderinfo.status==3">流水号:<span>{{cardbag.cardbag_number}}</span><view class="order-rdetails-information-copy" :data-index="cardbag.cardbag_number" @click="ctrlc">复制</view></view>
 		  		<view class="order-rdetails-information-li flex-vertically" v-else>礼包号:<span>{{cardbag.cardbag_number}}</span><view class="order-rdetails-information-copy" :data-index="cardbag.cardbag_number" @click="ctrlc">复制</view></view>
 	  ...
 	  <view v-else>
 		...
 		<view class="order-rdetails-commodity">
 			...
 			<view class="order-rdetails-commodity-content">
 				...
 				<!-- 批量,定时商品 -->
 				<view class="order-rdetails-commodity-li flex"  v-if="title_length>1" v-for="(item,index) in listt" :key="index"   @click="details" :data-index="item.keynum"  >
 					...
 					<view class="order-rdetails-commodity-right flex-wrap flex-vertically">
 						<view class="order-rdetails-commodity-circular" :data-index="index" v-if="right==index">
 							<view class="order-rdetails-commodity-nav" v-if="cardbag.status ==0">待支付</view>
 							<view class="order-rdetails-commodity-nav" v-else-if="cardbag.status ==1">待赠送</view>
 							<view class="order-rdetails-commodity-nav" v-else-if="cardbag.status ==2">赠送中</view>
 							<view class="order-rdetails-commodity-nav" v-else-if="cardbag.status ==3">已领取</view>
 							<view class="order-rdetails-commodity-nav" v-else-if="cardbag.status ==4">已退款</view>
 							<view class="order-rdetails-commodity-nav" v-else-if="cardbag.status ==5">已关闭</view>
 							<view class="order-rdetails-commodity-nav" v-else-if="cardbag.status ==6">待发货</view>
 							<view class="order-rdetails-commodity-nav" v-else-if="cardbag.status ==7">已发货</view>
 							<view class="order-rdetails-commodity-nav" v-else-if="cardbag.status ==8">已完成</view>
 						</view>
 						<view class="order-rdetails-commodity-price" v-if="level_name =='普通会员'" >¥{{item.price_level0}}</view>
 						<view class="order-rdetails-commodity-price" v-else-if="level_name =='指间会员'">¥{{item.price_level1}}</view>
 						<view class="order-rdetails-commodity-price"  v-else-if="level_name =='plus会员'">¥{{item.price_level2}}</view>
 						<view class="order-rdetails-commodity-price"  v-else-if="level_name =='企业会员'">¥{{item.price_level3}}</view>
 						<view class="order-rdetails-commodity-price"  v-else-if="level_name =='合作伙伴'">¥{{item.price_level4}}</view>
 						<view class="order-rdetails-commodity-price" v-else>¥{{item.price}}</view>
 				...
 				<!-- 直接,即时商品 -->
 				<view class="order-rdetails-commodity-li flex"  v-if="title_length<=1" v-for="(item,index) in list" :key="index"   @click="details" :data-index="item.keynum">
 					<view class="order-rdetails-commodity-img">
 						<image class="img" :src="$utils.imageUrl(item.head_img)"  v-if="item.head_img" mode=""></image>
 						<image class="img" src="../../static/nono.jpg"  v-else mode=""></image>
 					</view>
 			  ...
 			  <!-- 礼物份数 --> 
 				<view class="order-rdetails-gift1">
 					<view class="order-rdetails-gift" v-if ="cardbag.type==1">
 						直送/每份{{piece}}件礼物/礼包领取<span>{{cardbag.receive_details_num}}</span>
 						<view class="order-rdetails-gift-number"><span>x</span>{{cardbag.all_details_num}}份</view>
 					</view>
 					<view class="order-rdetails-gift" v-else-if ="cardbag.type==2">
 						批量/每份{{piece}}件礼物/礼包领取<span>{{cardbag.receive_details_num}}</span>
 						<view class="order-rdetails-gift-number"><span>x</span>{{cardbag.all_details_num}}份</view>
 					</view>
 					<view class="order-rdetails-gift" v-else-if ="cardbag.type==3">
 						定时/每份1件礼物/礼包领取<span>{{cardbag.receive_details_num}}</span>
 						<view class="order-rdetails-gift-number"><span>x</span>{{cardbag.all_details_num}}份</view>
 					</view>
 					<view class="order-rdetails-gift" v-else-if ="cardbag.type==4">
 						即时/每份1件礼物/礼包领取<span>{{cardbag.receive_details_num}}</span>
 						<view class="order-rdetails-gift-number"><span>x</span>{{cardbag.all_details_num}}份</view>
 					</view>
 				</view>
 				...
 				<!-- 优惠券 价格 -->
 				<view class="order-rdetails-btm flex-between">
 					<view class="order-rdetails-btm-title">优惠券</view>
 					<view class="order-rdetails-btm-alt">¥{{paylog.paycoupon}}</view>
 				</view>
 				<view class="order-rdetails-btm flex-between" v-if="paylog.paybalance>0.00">
 					<view class="order-rdetails-btm-title">余额</view>
 					<view class="order-rdetails-btm-alt">¥{{paylog.paybalance}}</view>
 				</view>
 				<view class="order-rdetails-btm flex-between" v-if="paylog.paybalance>0.00">
 					<view class="order-rdetails-btm-title">指尖币</view>
 					<view class="order-rdetails-btm-alt">¥{{paylog.paybalance_zj}}</view>
 				</view>
 				<view class="order-rdetails-btm flex-between" v-if="paylog.paybalance>0.00">
 					<view class="order-rdetails-btm-title">在线支付</view>
 					<view class="order-rdetails-btm-alt">¥{{paylog.paymoney}}</view>
 				</view>
 			</view>
 		...
 		<view class="order-rdetails-information">
 			<view class="order-rdetails-information-content">
 				<view class="order-rdetails-information-title">订单信息</view>
 				<view class="order-rdetails-information-li flex-vertically">礼包号:<span>{{cardbag.cardbag_number}}</span><view class="order-rdetails-information-copy" :data-index="cardbag.cardbag_number" @click="ctrlc">复制</view></view>
 				<view class="order-rdetails-information-li flex-vertically">创建时间:<span>{{cardbag.create_time}}</span></view>
 				<view class="order-rdetails-information-li flex-vertically">有效时间:<span>{{cardbag.exchange_time}}</span></view>
 			</view>
 		</view>
 			...
 			<view class="gift-partake-content">
 				...
 				<view class="gift-partake-li flex-between" v-for="(item,index) in partake" :key="index">
 					<view class="flex gift-partake-li-left">
 						<view class="gift-partake-li-portrait flex"><image :src="$utils.imageUrl(item.head_img)" mode=""></image>{{item.member_name}}</view>
 					</view>
 		...
 		<view v-if="status==0||status==1||status==3">
 			...
 			<!-- <view class="template-btm-information-voice" v-else-if="cardbag_theme.zhufu_type==1" @tap="mp3"><image class="img" src="../../static/luyin.jpg" mode=""></image></view> -->
 			 ...
 			 <!-- 小喇叭 -->
 			   <view class="img" @click="audioPlay" v-if="click==0"><image class="img"  src="../../static/luyinn.png"></image></view>
 			   ...
 			   <view class="box" @click="audioPlay"></view>
 			    ...
 			    <movable-area class="progress" :style="'width:'+w+'px'">
 			 								 <!-- 按钮 -->
 			       <movable-view direction="horizontal" damping="1000"  @change="scroll" @touchstart="star" @touchend="to" class="progress-one" :x="movable_x" >
 			         <view class="progress-view">
 			           <view class="progress-circular"></view>
 			 							 ...
 			 							 <!-- 进度条 -->
 			    <progress   :style="'width:'+w+'px'" :percent="schedule" class="b"  data-index="index"  activeColor='#BABABA' backgroundColor='#4E4E4E' border-radius="10" stroke-width="8"/>
 			 </view>
 			<!-- 视频 zhufu_mp4 -->
 			<view style="margin: 0 auto;width:537rpx;" v-else-if="cardbag_theme.zhufu_type==2"><video  class="gift-template-video" :src="$utils.imageUrl(cardbag_theme.zhufu_mp4)" controls></video></view>
 		</view>
 		...
 		<view class="gift-button" v-else-if="status == 2">
 		<!-- <view class="gift-button" v-else-if="status == 1 || status == 2 || status == 0"> -->
 			<view class="gift-button-give-top"></view>
 			<view class="gift-received-button flex-between">
 				<view class="gift-button-give gift-received-button-give" :data-cardbag_number="cardbag.cardbag_number"  :data-price="cardbag.price_discount" @click="give">立即转赠</view>
 			</view>
 		</view>
 		
 		<view class="gift-button" v-else-if="status == 0">
 		<!-- <view class="gift-button" v-else-if="status == 1 || status == 2 || status == 0"> -->
 			<view class="gift-button-give-top"></view>
 			<view class="gift-received-button flex-between">
 				<view class="gift-button-give" @click="immediately" :data-index="index" :data-cardbag_number="cardbag.cardbag_number" ><form @submit="submit" :data-cardbag_number="cardbag_number"><button  :data-index="index"  class="gift-button-give" :data-cardbag_number="cardbag_number" form-type="submit">立即支付</button></form></view>
 			</view>
 		</view>
 		...
 		<view class="gift-button" v-else>
 			<view class="gift-button-give-top"></view>
 			<view class="gift-received-button flex-between">
 				<view class="gift-button-give gift-received-button-give" style="background-color: #E27F8C;" :data-cardbag_number="cardbag.cardbag_number" @click="address">填写收货地址</view>
 				<view class="gift-button-give gift-received-button-give" :data-cardbag_number="cardbag.cardbag_number"  :data-price="cardbag.price_discount" @click="give">立即转增</view>
 			</view>
 		</view>
 	...
 	export default {
 		...
 		onLoad:function(e){
			...
			let data = JSON.stringify({
 			...
 			var dataa = '';
 			var aaction = 'get_payconfig';
 					 
 			this.$utils.post(aaction,dataa).then(res=>{
 		...
 		methods: { 
 			...
 			// 赠送好友
 			  ...
 			  // 我收到的赠送好友
 			    givee:function(e){
 				  ...
 				  var data = '{"cardbag_number":"'+this.cardbag_number+'","zhufu_theme_id":"'+this.cardbag_theme.id+'","memberid":"'+this.cardbag_theme.memberid+'"}';
 				  console.log(data)
 			...
 			submit:function(e){
 				let cardbag_number = e.currentTarget.dataset.cardbag_number;
 				console.log("cardbag_number", e.currentTarget.dataset.cardbag_number);
 				// 接口地址
 				...
 				// 请求
 				this.$utils.post(action, data).then(res => {
 					this.$utils.wxPay(res.rs.serial_number, "cardbag");
 				});
 			},
